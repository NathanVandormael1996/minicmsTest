...
class UsersRepository
{
    ...
    /**
     * findByExternalId()
     * Goal: Find a user linked to a specific external account[cite: 111, 112].
     */
    public function findByExternalId(string $provider, string $externalId): ?array
    {
        $sql = "SELECT u.id, u.email, u.name, u.is_active, r.name AS role_name
            FROM users u
            JOIN roles r ON r.id = u.role_id
            WHERE u.external_provider = :provider 
            AND u.external_id = :external_id
            AND u.is_active = 1
            LIMIT 1";
    ...
    /**
     * createExternal()
     * Goal: Create a new user from an external login[cite: 112, 117].
     * Note: password_hash is null or a random string as they use OAuth[cite: 128].
     */
    public function createExternal(string $email, string $name, string $provider, string $externalId, int $roleId): int
    {
        $sql = "INSERT INTO users (email, name, external_provider, external_id, role_id, is_active)
            VALUES (:email, :name, :provider, :external_id, :role_id, 1)";
