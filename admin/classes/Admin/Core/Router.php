...
class Router
{
    ...
    */
    ...
    private function compilePattern(string $path): string
    {
        ...
        // 1. Replace {id} with a digit-only regex
        $pattern = preg_replace('/\{id\}/', '(\d+)', $path);

        // 2. Replace {slug} with a regex that allows letters, numbers, and dashes
        $pattern = preg_replace('/\{slug\}/', '([a-z0-9-]+)', $pattern);

        // 3. Optional: A catch-all for any other word-based placeholder
